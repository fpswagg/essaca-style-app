  ...
  return (
    <section className="max-w-4xl mx-auto p-6 sm:p-8 bg-offwhite border border-main/10 rounded-2xl shadow-sm">
      <div className="flex flex-col gap-8">
        <div>
          <h3 className="text-xl font-display font-light text-main mb-2 bg-gradient-to-r from-magenta to-cyan bg-clip-text text-transparent">Admin Dashboard</h3>
        ...
        {/* Statistics Section */}
        <div>
          <h4 className="text-lg font-display font-light text-main mb-4">Statistics</h4>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            {Object.entries(stats).map(([key, value]) => (
              <div key={key} className="bg-gradient-to-br from-page to-offwhite rounded-2xl p-4 border border-main/10 text-center hover:border-magenta/30 transition-colors">
                <div className="text-2xl sm:text-3xl font-display font-light bg-gradient-to-r from-magenta to-cyan bg-clip-text text-transparent mb-1">
                  {value}
                </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {managementLinks.map((link) => (
              <div
                key={link.name}
                className="bg-gradient-to-br from-page to-offwhite rounded-2xl p-4 border border-main/10 hover:border-magenta/30 hover:shadow-md transition-all cursor-pointer"
              >
        ...
        <div>
          <h4 className="text-lg font-display font-light text-main mb-4">Applications Overview</h4>
          {applications.length === 0 ? (
            <div className="bg-gradient-to-br from-page to-offwhite rounded-2xl p-8 border border-main/10 text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {applications.map((app) => (
                <div
                  key={app.id}
                  className="bg-gradient-to-br from-page to-offwhite rounded-2xl p-4 border border-main/10 hover:border-magenta/30 hover:shadow-md transition-all"
                >
                  <div className="text-sm font-medium bg-gradient-to-r from-magenta to-cyan bg-clip-text text-transparent mb-2">Application #{app.id.slice(0, 8)}</div>
                  ...
                  <button className="w-full bg-gradient-to-r from-magenta to-cyan text-offwhite rounded-full px-4 py-2 text-sm font-medium hover:from-cyan hover:to-magenta transition-all">
                    View Details
                  </button>
                </div>
              ))}
