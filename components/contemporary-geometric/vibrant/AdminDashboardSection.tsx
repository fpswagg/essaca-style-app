  ...
  return (
    <section className="max-w-4xl mx-auto p-6 sm:p-8 bg-offwhite border-2 border-magenta/40 rounded-lg shadow-lg">
      <div className="flex flex-col gap-8">
        <div>
          <h3 className="text-xl font-display font-light text-magenta mb-2">Admin Dashboard</h3>
        ...
        {/* Statistics Section */}
        <div>
          <h4 className="text-lg font-display font-light text-magenta mb-4">Statistics</h4>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3">
            {Object.entries(stats).map(([key, value]) => (
              <div key={key} className="bg-page rounded-lg p-4 border-2 border-magenta/40 text-center hover:border-magenta/80 transition-colors shadow-md">
                <div className="text-2xl sm:text-3xl font-display font-light text-magenta mb-1">
                  {value}
                </div>
                <div className="text-xs sm:text-sm text-main/70 capitalize font-medium">
        ...
        {/* Management Links */}
        <div>
          <h4 className="text-lg font-display font-light text-magenta mb-4">Management Links</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
            {managementLinks.map((link) => (
              <div
                key={link.name}
                className="bg-page rounded-lg p-4 border-2 border-magenta/40 hover:border-magenta/80 hover:shadow-lg transition-all cursor-pointer shadow-md"
              >
                <h5 className="font-medium text-magenta mb-1">{link.name}</h5>
        ...
        <div>
          <h4 className="text-lg font-display font-light text-magenta mb-4">Applications Overview</h4>
          {applications.length === 0 ? (
            <div className="bg-page rounded-lg p-8 border-2 border-magenta/40 text-center shadow-md">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3">
              {applications.map((app) => (
                <div
                  key={app.id}
                  className="bg-page rounded-lg p-4 border-2 border-magenta/40 hover:border-magenta/80 hover:shadow-lg transition-all shadow-md"
                >
                  <div className="text-sm font-medium text-magenta mb-2">Application #{app.id.slice(0, 8)}</div>
                  ...
                  <button className="w-full bg-magenta text-offwhite rounded-full px-4 py-2 text-sm font-medium hover:bg-cyan transition-colors shadow-md">
                    View Details
                  </button>
                </div>
              ))}
