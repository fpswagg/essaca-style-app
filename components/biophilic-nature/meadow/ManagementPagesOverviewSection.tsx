        ...
        <div className="mt-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {managementPages.map((page) => (
              <div
                key={page.name}
                className="bg-page rounded-2xl p-5 border border-main/10 hover:border-olive/30 hover:shadow-md transition-all"
              >
                <div className="flex justify-between items-start mb-2">
                  <h4 className="font-medium text-main">{page.name}</h4>
                  <span className="text-xs px-2 py-1 bg-olive/10 text-olive rounded-full">
                    {page.access}
                  </span>
                </div>
