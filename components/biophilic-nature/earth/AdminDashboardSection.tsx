        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            {Object.entries(stats).map(([key, value]) => (
              <div key={key} className="bg-page rounded-lg p-4 border border-main/20 text-center hover:border-earth/50 transition-colors">
                <div className="text-2xl sm:text-3xl font-display font-light text-earth mb-1">
                  {value}
                </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {managementLinks.map((link) => (
              <div
                key={link.name}
                className="bg-page rounded-lg p-4 border border-main/20 hover:border-earth/50 hover:shadow-md transition-all cursor-pointer"
              >
          ...
          {applications.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {applications.map((app) => (
                <div
                  key={app.id}
                  className="bg-page rounded-lg p-4 border border-main/20 hover:border-earth/50 hover:shadow-md transition-all"
                >
                  <div className="text-sm font-medium text-earth mb-2">Application #{app.id.slice(0, 8)}</div>
                  ...
                  <button className="w-full bg-earth text-cream rounded-full px-4 py-2 text-sm font-medium hover:bg-olive transition-colors">
                    View Details
                  </button>
                </div>
              ))}
