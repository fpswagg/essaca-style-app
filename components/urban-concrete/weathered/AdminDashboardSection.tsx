      ...
      <div className="flex flex-col gap-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            {Object.entries(stats).map(([key, value]) => (
              <div key={key} className="bg-page rounded-lg p-4 border border-main/20 text-center hover:border-steel/40 transition-colors opacity-95">
                <div className="text-2xl sm:text-3xl font-display font-light text-steel mb-1">
                  {value}
                </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {managementLinks.map((link) => (
              <div
                key={link.name}
                className="bg-page rounded-lg p-4 border border-main/20 hover:border-steel/40 hover:shadow-md transition-all cursor-pointer opacity-95"
              >
        ...
        <div>
          ...
          {applications.length === 0 ? (
            <div className="bg-page rounded-lg p-8 border border-main/20 text-center opacity-95">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {applications.map((app) => (
                <div
                  key={app.id}
                  className="bg-page rounded-lg p-4 border border-main/20 hover:border-steel/40 hover:shadow-md transition-all opacity-95"
                >
                  ...
                  <button className="w-full bg-steel text-card rounded-full px-4 py-2 text-sm font-medium hover:bg-concrete transition-colors">
                    View Details
                  </button>
                </div>
              ))}
