        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            {Object.entries(stats).map(([key, value]) => (
              <div key={key} className="bg-page rounded-2xl p-4 border border-main/10 text-center hover:border-burgundy/30 transition-colors shadow-sm">
                <div className="text-2xl sm:text-3xl font-display font-light text-burgundy mb-1">
                  {value}
                </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {managementLinks.map((link) => (
              <div
                key={link.name}
                className="bg-page rounded-2xl p-4 border border-main/10 hover:border-burgundy/30 hover:shadow-md transition-all cursor-pointer"
              >
          ...
          {applications.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {applications.map((app) => (
                <div
                  key={app.id}
                  className="bg-page rounded-2xl p-4 border border-main/10 hover:border-burgundy/30 hover:shadow-md transition-all"
                >
