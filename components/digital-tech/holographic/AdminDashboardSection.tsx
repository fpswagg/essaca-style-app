  ...
  return (
    <section className="max-w-4xl mx-auto p-6 sm:p-8 bg-panel/80 backdrop-blur-sm border border-neon/30 rounded-2xl shadow-lg">
      <div className="flex flex-col gap-8">
        <div>
          <h3 className="text-xl font-display font-light text-neon mb-2 drop-shadow-[0_0_6px_rgba(0,255,136,0.3)]">Admin Dashboard</h3>
        ...
        {/* Statistics Section */}
        <div>
          <h4 className="text-lg font-display font-light text-neon mb-4 drop-shadow-[0_0_4px_rgba(0,255,136,0.3)]">Statistics</h4>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            {Object.entries(stats).map(([key, value]) => (
              <div key={key} className="bg-page/50 backdrop-blur-sm rounded-2xl p-4 border border-neon/30 text-center hover:border-neon/60 transition-colors shadow-md">
                <div className="text-2xl sm:text-3xl font-display font-light text-neon mb-1 drop-shadow-[0_0_6px_rgba(0,255,136,0.4)]">
                  {value}
                </div>
                <div className="text-xs sm:text-sm text-main/70 capitalize">
        ...
        {/* Management Links */}
        <div>
          <h4 className="text-lg font-display font-light text-neon mb-4 drop-shadow-[0_0_4px_rgba(0,255,136,0.3)]">Management Links</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {managementLinks.map((link) => (
              <div
                key={link.name}
                className="bg-page/50 backdrop-blur-sm rounded-2xl p-4 border border-neon/30 hover:border-neon/60 hover:shadow-md transition-all cursor-pointer shadow-md"
              >
                <h5 className="font-medium text-neon mb-1 drop-shadow-[0_0_3px_rgba(0,255,136,0.3)]">{link.name}</h5>
        ...
        <div>
          <h4 className="text-lg font-display font-light text-neon mb-4 drop-shadow-[0_0_4px_rgba(0,255,136,0.3)]">Applications Overview</h4>
          {applications.length === 0 ? (
            <div className="bg-page/50 backdrop-blur-sm rounded-2xl p-8 border border-neon/30 text-center shadow-md">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {applications.map((app) => (
                <div
                  key={app.id}
                  className="bg-page/50 backdrop-blur-sm rounded-2xl p-4 border border-neon/30 hover:border-neon/60 hover:shadow-md transition-all shadow-md"
                >
                  <div className="text-sm font-medium text-neon mb-2 drop-shadow-[0_0_4px_rgba(0,255,136,0.4)]">Application #{app.id.slice(0, 8)}</div>
                  ...
                  <button className="w-full bg-neon/80 text-page rounded-full px-4 py-2 text-sm font-medium hover:bg-electric transition-colors backdrop-blur-sm shadow-[0_0_8px_rgba(0,255,136,0.4)]">
                    View Details
                  </button>
                </div>
              ))}
