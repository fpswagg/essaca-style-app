  ...
  return (
    <section className="max-w-4xl mx-auto p-6 sm:p-8 bg-panel border border-neon/50 rounded-none shadow-sm">
      <div className="flex flex-col gap-8">
        <div>
          <h3 className="text-xl font-display font-light text-main mb-2 font-mono">ManagementPagesOverview</h3>
        ...
        <div className="mt-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {managementPages.map((page) => (
              <div
                key={page.name}
                className="bg-page rounded-none p-5 border border-neon/50 hover:border-neon/30 hover:shadow-md transition-all"
              >
                <div className="flex justify-between items-start mb-2">
                  <h4 className="font-medium text-main font-mono">{page.name}</h4>
                  <span className="text-xs px-2 py-1 bg-neon/10 text-neon rounded-full">
                    {page.access}
                  </span>
                </div>
                <p className="text-xs sm:text-sm text-main/70 mb-3 font-mono">{page.description}</p>
                <ul className="space-y-1">
                  {page.features.map((feature, idx) => (
                    <li key={idx} className="text-xs text-main/60 flex items-center gap-2 font-mono">
