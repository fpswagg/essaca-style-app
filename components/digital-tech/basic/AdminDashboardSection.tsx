...
export default function AdminDashboardSection() {
  ...
  return (
    <section className="max-w-4xl mx-auto p-6 sm:p-8 bg-panel border border-main/20 rounded-lg shadow-sm">
      <div className="flex flex-col gap-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            {Object.entries(stats).map(([key, value]) => (
              <div key={key} className="bg-page rounded-lg p-4 border border-main/20 text-center hover:border-neon/50 transition-colors">
                <div className="text-2xl sm:text-3xl font-display font-light text-neon mb-1">
                  {value}
                </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {managementLinks.map((link) => (
              <div
                key={link.name}
                className="bg-page rounded-lg p-4 border border-main/20 hover:border-neon/50 hover:shadow-md transition-all cursor-pointer"
              >
        ...
        <div>
          ...
          {applications.length === 0 ? (
            <div className="bg-page rounded-lg p-8 border border-main/20 text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {applications.map((app) => (
                <div
                  key={app.id}
                  className="bg-page rounded-lg p-4 border border-main/20 hover:border-neon/50 hover:shadow-md transition-all"
                >
                  <div className="text-sm font-medium text-neon mb-2">Application #{app.id.slice(0, 8)}</div>
                  ...
                  <button className="w-full bg-neon text-page rounded-full px-4 py-2 text-sm font-medium hover:bg-electric transition-colors">
                    View Details
                  </button>
                </div>
              ))}
