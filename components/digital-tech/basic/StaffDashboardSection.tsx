  ...
  return (
    <section className="max-w-4xl mx-auto p-6 sm:p-8 bg-panel border border-main/10 rounded-lg shadow-sm">
      <div className="flex flex-col gap-8">
        <div>
          <h3 className="text-xl font-display font-light text-neon mb-2 drop-shadow-[0_0_6px_rgba(0,255,136,0.4)]">Staff Dashboard</h3>
        ...
        {/* Management Links */}
        <div>
          <h4 className="text-lg font-display font-light text-neon mb-4 drop-shadow-[0_0_4px_rgba(0,255,136,0.4)]">Management Links</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {managementLinks.map((link) => (
              <div
                key={link.name}
                className="bg-page rounded-lg p-4 border border-neon/30 hover:border-neon/60 hover:shadow-md transition-all cursor-pointer"
              >
                <h5 className="font-medium text-neon mb-1 drop-shadow-[0_0_3px_rgba(0,255,136,0.4)]">{link.name}</h5>
        ...
        {/* Applications List */}
        <div>
          <h4 className="text-lg font-display font-light text-neon mb-4 drop-shadow-[0_0_4px_rgba(0,255,136,0.4)]">All Applications</h4>
          {applications.length === 0 ? (
            <div className="bg-page rounded-lg p-8 border border-neon/30 text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {applications.map((app) => (
                <div
                  key={app.id}
                  className="bg-page rounded-lg p-4 border border-neon/30 hover:border-neon/60 hover:shadow-md transition-all"
                >
                  ...
                  <button className="w-full bg-neon text-page rounded-full px-4 py-2 text-sm font-medium hover:bg-electric transition-colors shadow-[0_0_8px_rgba(0,255,136,0.4)]">
                    View Details
                  </button>
                </div>
              ))}
